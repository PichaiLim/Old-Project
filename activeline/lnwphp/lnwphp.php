...
define('lnwphp_PATH', str_replace('\\', '/', dirname(__file__)));
require (lnwphp_PATH . '/lnwphp_config.php');
require (lnwphp_PATH . '/lnwphp_db.php');
require (lnwphp_PATH . '/lnwphp_dbpdo.php');
mb_internal_encoding(lnwphp_config::$mbencoding);
...
class lnwphp
{
    ...
    protected $upload_folder_def = '../image';
    ...
    protected $load_view = array(
        'list' => 'lnwphp_list_view.php',
        'create' => 'lnwphp_detail_view.php',
        'edit' => 'lnwphp_detail_view.php',
        'view' => 'lnwphp_detail_view.php');
    ...
    {
        lnwphp_config::$scripts_url = self::check_url(lnwphp_config::$scripts_url, true);
        lnwphp_config::$editor_url = self::check_url(lnwphp_config::$editor_url);
        lnwphp_config::$editor_init_url = self::check_url(lnwphp_config::$editor_init_url);

        $this->limit = lnwphp_config::$limit;
        $this->limit_list = lnwphp_config::$limit_list;
        $this->column_cut = lnwphp_config::$column_cut;
        $this->show_primary_ai_field = lnwphp_config::$show_primary_ai_field;
        $this->show_primary_ai_column = lnwphp_config::$show_primary_ai_column;

        $this->benchmark = lnwphp_config::$benchmark;
        $this->start_minimized = lnwphp_config::$start_minimized;
        $this->remove_confirm = lnwphp_config::$remove_confirm;
        $this->upload_folder_def = lnwphp_config::$upload_folder_def;

        $this->theme = lnwphp_config::$theme;
        $this->is_print = lnwphp_config::$enable_printout;
        $this->is_title = lnwphp_config::$enable_table_title;
        $this->is_csv = lnwphp_config::$enable_csv_export;
        $this->is_numbers = lnwphp_config::$enable_numbers;
        $this->is_pagination = lnwphp_config::$enable_pagination;
        $this->is_search = lnwphp_config::$enable_search;
        $this->is_limitlist = lnwphp_config::$enable_limitlist;
        $this->is_sortable = lnwphp_config::$enable_sorting;

        $this->language = lnwphp_config::$language;

        $this->search_pattern = lnwphp_config::$search_pattern;

        $this->demo_mode = lnwphp_config::$demo_mode;

        $this->default_tab = lnwphp_config::$default_tab;
        $this->prefix = lnwphp_config::$dbprefix;
        $this->is_rtl = lnwphp_config::$is_rtl;

        $this->strip_tags = lnwphp_config::$strip_tags;
        $this->safe_output = lnwphp_config::$safe_output;
    }
    ...
    {
        if (isset($_POST['lnwphp']['instance']) && isset($_POST['lnwphp']['key']) && isset($_POST['lnwphp']['task']))
        {
            self::init_prepare('post');
            $key = $_POST['lnwphp']['key'] ? $_POST['lnwphp']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_POST['lnwphp']['instance'] ? $_POST['lnwphp']['instance'] : self::error('Instance name cannot be empty');
            $is_get = false;
        }
        elseif (isset($_GET['lnwphp']['instance']) && isset($_GET['lnwphp']['key']) && isset($_GET['lnwphp']['task']) && $_GET['lnwphp']['task'] ==
            'file')
        {
            self::init_prepare('get');
            $key = $_GET['lnwphp']['key'] ? $_GET['lnwphp']['key'] : self::error('Security key cannot be empty');
            $inst_name = $_GET['lnwphp']['instance'] ? $_GET['lnwphp']['instance'] : self::error('Instance name cannot be empty');
            $is_get = true;
        }
        else
        {
        ...
        if (isset($_SESSION['lnwphp_session'][$inst_name]['key']) && $_SESSION['lnwphp_session'][$inst_name]['key'] == $key)
        {
    ...
    protected static function init_prepare($method = false)
    {
        ...
        {
            case 'post':
            $sess_name = (lnwphp_config::$dynamic_session && isset($_POST['lnwphp']['sess_name']) && $_POST['lnwphp']['sess_name']) ? $_POST['lnwphp']['sess_name'] :
            lnwphp_config::$sess_name;
            break;
            case 'get':
            $sess_name = (lnwphp_config::$dynamic_session && isset($_GET['lnwphp']['sess_name']) && $_GET['lnwphp']['sess_name']) ? $_GET['lnwphp']['sess_name'] :
            lnwphp_config::$sess_name;
            break;
            default:
            $sess_name = lnwphp_config::$sess_name;
            break;
        }
        self::session_start($sess_name);
        if (is_callable(lnwphp_config::$before_construct))
        {
            call_user_func(lnwphp_config::$before_construct);
        }
    }
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (!session_id() && !lnwphp_config::$external_session && !lnwphp_config::$alt_session)
        {
            if (!headers_sent())
            {
                session_name($sess_name);
                session_cache_expire(lnwphp_config::$sess_expire);
                session_set_cookie_params(0, '/');
                session_start();
            }
            else
                self::error('lnwphp can not create session, because the output is already sent into browser. 
                    Try to define lnwphp instance before the output start or use session_start() at the beginning of your script');
        }
        if (lnwphp_config::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                setcookie($sess_name, $this->sess_id, time() + lnwphp_config::$alt_lifetime * 60, '/');
            }
            else
                self::error('lnwphp can not start session, because the output is already sent into browser. 
                    Try to define lnwphp instance before the output start or use <strong>lnwphp::session_start();</strong> at the beginning of your script');
        }
    }
    ...
    function table_array_all($table='',$where='',$limit='')
    {
       $db = new lnwphp_dbpdo;
       $array = $db->fetchAll("SELECT * FROM `$table` LIMIT 10");
       if ($where != '' && $limit != '') {
           $array = $db->fetchAll("SELECT * FROM `$table` WHERE $where LIMIT $limit");
       }elseif ($where != '') {
           $array = $db->fetchAll("SELECT * FROM `$table` WHERE $where");
       }elseif ($limit != '') {
        $array = $db->fetchAll("SELECT * FROM `$table` LIMIT $limit");
    }
    return $array;
}
    function table_array_all_D($table='',$where='',$limit='',$d='')
    {
       $db = new lnwphp_dbpdo;
       $array = $db->fetchAll("SELECT DISTINCT $d FROM `$table` LIMIT 10");
       if ($where != '' && $limit != '') {
           $array = $db->fetchAll("SELECT DISTINCT $d FROM `$table` WHERE $where LIMIT $limit");
       }elseif ($where != '') {
           $array = $db->fetchAll("SELECT DISTINCT $d FROM `$table` WHERE $where");
       }elseif ($limit != '') {
        $array = $db->fetchAll("SELECT DISTINCT $d FROM `$table` LIMIT $limit");
    }
    return $array;
}
function table_array($table='',$where='',$limit='')
{
   $db = new lnwphp_dbpdo;
   $array = $db->fetch("SELECT * FROM `$table` LIMIT 10");
   if ($where != '' && $limit != '') {
       $array = $db->fetch("SELECT * FROM `$table` WHERE $where LIMIT $limit");
   }elseif ($where != '') {
       $array = $db->fetch("SELECT * FROM `$table` WHERE $where");
   }elseif ($limit != '') {
    $array = $db->fetch("SELECT * FROM `$table` LIMIT $limit");
}
return $array;
}
...
function lnw_substr($str='',$start='',$end='')
{
    $str = mb_substr(strip_tags($str),$start,$end,'UTF-8');
    if (strlen($str) > $end) {
        $str = $str.'...';
    }
...
function get_youtube_thumb($link='',$type= 'sddefault'){
    ...
    if($type == 'default' || $type == 'hqdefault' || $type == 'mqdefault' || $type == 'sddefault' || $type == 'maxresdefault'){
        ...
        $thumb_link = '//img.youtube.com/vi/'.$video_id.'/'.$type.'.jpg';
...
function playyoutube($url='',$height='480',$width='640')
{
    preg_match('/[\\?\\&]v=([^\\?\\&]+)/', $url, $matches);
    $id = $matches[1];
    $code = '<div class="embed-responsive embed-responsive-16by9"><iframe class="embed-responsive-item" width="'.$width.'" height="'.$height.'" src="//www.youtube.com/embed/'.$id.'" frameborder="0" allowfullscreen></iframe></div>';
    return $code;
}
function re_url($url="เทพ พี เฮส พี lnwphp"){
    $url = strtolower(str_replace(" ","-",$url));
    $url = strtolower(preg_replace('~[^a-z0-9ก-๙\.\-\_]~iu','',$url));
    return $url ;
}
function _pass($pass) {
        return md5(sha1(base64_encode(md5($pass))));
    }
...
public function nested_table($instance_name = '', $field = '', $inner_tbl = '', $tbl_field = '')
{
    if ($instance_name && $field && $inner_tbl && $tbl_field)
    {
        ...
        foreach ($fdata as $fitem)
        {
            ...
            $instance = lnwphp::get_instance($instance_name);
    ...
    if ($type)
    {
        ...
        foreach ($fdata as $fitem)
        {
            ...
            switch ($type)
            {
                ...
                case 'point':
                $def_attr = array(
                    'text' => lnwphp_config::$default_text,
                    'search_text' => lnwphp_config::$default_search_text,
                    'zoom' => lnwphp_config::$default_zoom,
                    'width' => lnwphp_config::$default_width,
                    'height' => lnwphp_config::$default_height,
                    'search' => lnwphp_config::$default_coord,
                    'coords' => lnwphp_config::$default_search);
                $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = array_merge($def_attr, (array )$attr);
                break;
                case 'remote_image':
...
protected function _run_task()
{
    ...
    switch ($this->task)
    {
        ...
        $this->_set_field_types('list', lnwphp_config::$print_all_fields);
        $this->theme = 'printout';
        return $this->_list();
        break;
        case 'depend':
        ...
        break;
        case 'csv':
        $this->_set_field_types('list', lnwphp_config::$csv_all_fields);
        return $this->_csv();
        break;
        case 'list':
        default:
...
protected function render_custom_datagrid()
{
    ...
    $db = lnwphp_db::get_instance($this->connection);
...
protected function render_custom_csv()
{
    ...
    fputcsv($output, $this->columns_names, lnwphp_config::$csv_delimiter, lnwphp_config::$csv_enclosure);
    ...
    while ($row = $db->result->fetch_assoc())
    {
        ...
        fputcsv($output, $out, lnwphp_config::$csv_delimiter, lnwphp_config::$csv_enclosure);
    }
}
...
protected function render_output()
{
    ...
    {
        $contents = '';
        if (!self::$css_loaded && !lnwphp_config::$manual_load)
        {
            $contents .= self::load_css();
        }
        ob_start();
        include (lnwphp_PATH . '/' . lnwphp_config::$themes_path . '/' . $this->theme . '/lnwphp_container.php');
        $contents .= ob_get_contents();
        ob_end_clean();
        unset($this->data);
        if (!self::$js_loaded && !lnwphp_config::$manual_load)
        {
...
protected function after_render()
{
    ...
    if (is_callable(lnwphp_config::$after_render))
    {
        call_user_func(lnwphp_config::$after_render);
    }
}
...
public function _csv()
{
    ...
    $output = fopen('php://output', 'w');
    fwrite($output, chr(0xEF) . chr(0xBB) . chr(0xBF));
    fputcsv($output, $headers, lnwphp_config::$csv_delimiter, lnwphp_config::$csv_enclosure);
...
{
    if (isset($_POST['lnwphp'][$field]))
    {
        if (lnwphp_config::$auto_xss_filtering)
        {
        ...
        if (($field == 'postdata' or $field == 'unique') && $_POST['lnwphp'][$field])
        {
            $data_keys = array_keys($_POST['lnwphp'][$field]);
            foreach ($data_keys as $k => $key)
            {
                $data_keys[$k] = $xss ? $xss->xss_clean($this->fieldname_decode($key)) : $this->fieldname_decode($key);
                if ($xss)
                {
                    $_POST['lnwphp'][$field][$key] = $xss->xss_clean($_POST['lnwphp'][$field][$key]);
                }
            }
            return array_combine($data_keys, $_POST['lnwphp'][$field]);
        }
        elseif ($filter)
        {
            switch ($filter)
            {
                case 'key':
                return str_replace('`', '', $xss ? $xss->xss_clean($_POST['lnwphp'][$field]) : $_POST['lnwphp'][$field]);
                break;
                case 'int':
                return (int)$_POST['lnwphp'][$field];
                break;
                case 'trim':
                return trim($xss ? $xss->xss_clean($_POST['lnwphp'][$field]) : $_POST['lnwphp'][$field]);
                break;
                case 'base64':
                return $xss ? $xss->xss_clean($this->fieldname_decode($_POST['lnwphp'][$field])) : $this->fieldname_decode($_POST['lnwphp'][$field]);
                break;
                default:
                return $xss ? $xss->xss_clean($_POST['lnwphp'][$field]) : $_POST['lnwphp'][$field];
                break;
            }
        }
        else
...
protected function _get($field = '', $default = false, $filter = false)
{
    if (isset($_GET['lnwphp'][$field]))
    {
        ...
        if ($filter)
        {
            ...
            {
                case 'key':
                return str_replace('`', '', $xss ? $xss->xss_clean($_GET['lnwphp'][$field]) : $_GET['lnwphp'][$field]);
                break;
                case 'int':
                return (int)$_GET['lnwphp'][$field];
                break;
                case 'trim':
                return trim($xss ? $xss->xss_clean($_GET['lnwphp'][$field]) : $_GET['lnwphp'][$field]);
                break;
                default:
                return $xss ? $xss->xss_clean($_GET['lnwphp'][$field]) : $_GET['lnwphp'][$field];
                break;
            }
        }
        else
    ...
    if (!$this->primary_val)
    {
        ...
        if ($this->alert_create)
        {
            foreach ($this->alert_create as $alert)
            {
                ...
                if (lnwphp_config::$email_enable_html)
                    $message = $alert['message'] . '<br /><br />' . "\r\n" . ($alert['link'] ? '<a href="' . $alert['link'] .
                        '" target="_blank">' . $alert['link'] . '</a>' : '');
                else
                    $message = $alert['message'] . "\r\n\r\n" . ($alert['link'] ? $alert['link'] : '');
                $this->send_email($postdata[$alert['column']], $alert['subject'], $message, $alert['cc'], lnwphp_config::$email_enable_html);
            }
        }
        if ($this->mass_alert_create)
        {
            foreach ($this->mass_alert_create as $alert)
            {
                ...
                {
                    $this->send_email($row[$alert['email_column']], $alert['subject'], $message, array(), lnwphp_config::$email_enable_html);
                }
            }
        }
        $pd = new lnwphp_postdata($postdata, $this);
...
protected function _receive_post($task = false, $primary = false)
{
    ...
    else
    {
        ...
        $this->limit = $this->_post('limit', ($this->limit ? $this->limit : lnwphp_config::$limit));
...
protected function _define_field_type($row, $field_index)
{
    ...
    switch ($type)
    {
        ...
        case 'bool':
        case 'boolean':
        if ($max_l == 1 && lnwphp_config::$make_checkbox)
        {
        ...
        case 'mediumtext':
        case 'longtext':
        if (!isset($this->no_editor[$field_index]) && lnwphp_config::$auto_editor_insertion)
        ...
        break;
        case 'enum':
        $this->field_type[$field_index] = lnwphp_config::$enum_as_radio ? 'radio' : 'select';
        $this->field_attr[$field_index]['values'] = $max_l;
        if (!isset($this->defaults[$field_index]))
            $this->defaults[$field_index] = $row['Default'];
        break;
        case 'set':
        $this->field_type[$field_index] = lnwphp_config::$set_as_checkboxes ? 'checkboxes' : 'multiselect';
        $this->field_attr[$field_index]['values'] = $max_l;
        if (!isset($this->defaults[$field_index]))
            $this->defaults[$field_index] = $row['Default'];
        break;
        ...
        if (!isset($this->defaults[$field_index]))
            $this->defaults[$field_index] = lnwphp_config::$default_point ? lnwphp_config::$default_point : '0,0';
        break;
    }
}
...
protected function _render_list()
{
    ...
    if ($this->column === false)
    {
        ...
        elseif (!lnwphp_config::$search_all)
        {
            if ($this->search_columns)
            {
    ...
    $mode = 'list';
    $view_file = lnwphp_PATH . '/' . lnwphp_config::$themes_path . '/' . $this->theme . '/' . $this->load_view['list'];
    ...
    if ($this->inner_table_instance && ($mode == 'view' or $mode == 'edit'))
    {
        foreach ($this->inner_table_instance as $inst_name => $field)
        {
            if (isset($this->result_row[$field]))
            {
                ...
                if ($mode == 'view' && lnwphp_config::$nested_readonly_on_view)
                {
                ...
                '<div class="lnwphp-nested-container lnwphp-container"><div class="lnwphp-ajax" id="lnwphp-ajax-' . base_convert(rand(), 10,
                    36) . '">' . $instance->render('list') . '</div></div>';
            }
        }
    }

    $view_file = lnwphp_PATH . '/' . lnwphp_config::$themes_path . '/' . $this->theme . '/' . $this->load_view[$mode];
    $this->check_file($view_file, 'render');
    ob_start();
    include ($view_file);
...
protected function _pagination($total, $start, $limit, $numpos = 10, $numlr = 2)
{
    ...
    protected function _pagination_item($i, $curent, $limit)
    {
        ...
        else
        {
            ...
            '"><a href="javascript:;" class="lnwphp-action" data-start="' . $limit1 . '">' . $i . '</a></li>';
        }
    }
    protected function _cut($string, $field, $wordsafe = true, $dots = true)
    {
        ...
        $string = html_entity_decode($string, ENT_QUOTES, lnwphp_config::$mbencoding);
        ...
        $slen = mb_strlen($strip_string, lnwphp_config::$mbencoding);
        if ($slen <= $len)
        {
            return $this->output_string($string, $this->strip_tags, $safe);
        }
        ...
        {
            $end = $len;
            while ((mb_substr($strip_string, --$len, 1, lnwphp_config::$mbencoding) != ' ') && ($len > 0))
            {
            }
            if ($len == 0)
            {
            ...
            return $this->output_string(mb_substr($strip_string, 0, $len, lnwphp_config::$mbencoding), false, $safe) . ($dots ?
                '&#133;' : '');
        }
        return $this->output_string(mb_substr($strip_string, 0, $len, lnwphp_config::$mbencoding), false, $safe) . ($dots ?
            '&#133;' : '');
    ...
    {
        return mb_convert_case(str_replace('_', ' ', $text), MB_CASE_TITLE, lnwphp_config::$mbencoding);
    }
    ...
    public function _export_vars()
    {
        ...
        {
            if (isset($_SESSION['lnwphp_session']) && $_SESSION['lnwphp_session'])
            {
                foreach ($_SESSION['lnwphp_session'] as $s_key => $s_val)
                {
                    $old_time = isset($s_val['time']) ? (int)$s_val['time'] : 0;
                    if ($time > $old_time + lnwphp_config::$autoclean_timeout)

                        unset($_SESSION['lnwphp_session'][$s_key]);
                }
            }
        }
        ...
        {
            $_SESSION['lnwphp_session'][$inst_name][$item] = $this->{$item};
        }
        $_SESSION['lnwphp_session'][$inst_name]['before'] = $this->find_prev_task();

        if (lnwphp_config::$alt_session)
        {
            $data = $this->encrypt($_SESSION['lnwphp_session']);

            if (class_exists('Memcache'))
            {
                $mc = new Memcache();
                $mc->connect(lnwphp_config::$mc_host, lnwphp_config::$mc_port);
                $res = $mc->set($this->sess_id, $data, false, lnwphp_config::$alt_lifetime * 60);
            }
            elseif (class_exists('Memcached'))
            {
                $mc = new Memcached();
                $mc->connect(lnwphp_config::$mc_host, lnwphp_config::$mc_port);
                $res = $mc->set($this->sess_id, $data, lnwphp_config::$alt_lifetime * 60);
            }
            else
            {
                self::error('Can\'t use alternative session. Memcache(d) is not available');
            }
            unset($_SESSION['lnwphp_session']);
            if (!$res)
            {
...
public function import_vars($key = false)
{
    if (lnwphp_config::$alt_session)
    {
        ...
        $_SESSION['lnwphp_session'] = $this->decrypt($data[0], $data[1]);
        unset($data);
        if (!$_SESSION['lnwphp_session'])
        {
    ...
    foreach ($this->params2save() as $item)
    {
        $this->{$item} = $_SESSION['lnwphp_session'][$inst_name][$item];
    }
...
{
    return '<span class="lnwphp-none">' . $value . '</span>';
}
...
{
    if (lnwphp_config::$clickable_list_links)
    {
...
protected function create_radio($name, $value = '', $tag = array())
{
    ...
    $label_tag = array('tag' => 'label', 'class' => 'lnwphp-radio-label');
...
protected function create_checkboxes($name, $value = '', $tag = array())
{
    ...
    $label_tag = array('tag' => 'label', 'class' => 'lnwphp-checkboxes-label');
...
protected function create_hidden($name, $value = '', $tag = array())
{
    return $this->single_tag($tag + array(
        ...
        'type' => 'hidden',
        'value' => $value,
        'name' => $name), 'lnwphp-input', $this->field_attr[$name], true);
}
...
protected function create_relation($name, $value = '', $tag = array(), $dependval = false)
{
    ...
    {
        $options = false;
        if (lnwphp_config::$lists_null_opt)
        {
            foreach ($values as $val)
            {
...
protected function create_file($name, $value = '', $tag = array(), $is_upload = false)
{
    ...
    $out = '';
    $out .= $this->open_tag('div', 'lnwphp-upload-container');
    ...
    if ($value)
    {
        ...
        $attr = array(
            ...
            'class' => 'lnwphp-file-name lnwphp-' . $ext,
            'target' => '_blank');
    ...
    {
        $out .= $this->open_tag('span', 'lnwphp-nofile ' . $this->theme_config('no_file'));
    ...
    if (!isset($tag['readonly']) && !isset($tag['disabled']))
    {
        $attr = array(
            ...
            'value' => $value,
            'type' => 'hidden');
        $out .= $this->single_tag('input', 'lnwphp-input', $attr, true);
...
{
    $out = '';
    $out .= $this->open_tag('span', $this->theme_config('upload_button'), array('class' => 'lnwphp-add-file'));
...
protected function create_view_file($name, $value = '', $tag = array(), $is_upload = false)
{
    ...
    if ($value)
    {
        ...
        $attr = array(
            'href' => $this->file_link($name, $this->primary_val),
            'class' => 'lnwphp-file lnwphp-' . $ext,
            'target' => '_blank');
        $out .= $this->open_tag('span', 'lnwphp-file-name');
    ...
    {
        $out .= $this->open_tag('span', 'lnwphp-nofile');
        $out .= $this->lang('no_file') . $this->close_tag('span');
    }
    return $out;
}
protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
{
    ...
    {
        $out .= $this->open_tag('div', $this->theme_config('grid_button_group'));
        $out .= $this->open_tag('span', 'lnwphp-noimage ' . $this->theme_config('no_file'));
...
protected function create_view_image($name, $value = '', $tag = array())
{
    ...
    {
        $out .= $this->open_tag('span', 'lnwphp-noimage');
        $out .= $this->lang('no_image') . $this->close_tag('span');
    }
...
protected function create_point($name, $value = '', $tag = array())
{
    ...
    if (!$value)
    {
        $value = lnwphp_config::$default_point ? lnwphp_config::$default_point : '0,0';
    }
...
protected function create_view_point($name, $value = '', $tag = array())
{
    ...
    if (!$value)
    {
        $value = lnwphp_config::$default_point;
    }
    if ($value)
    {
        $tag = array(
...
{
    exit('<div class="lnwphp-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
        <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">lnwPHP.in.th lnwphp</span>' . $text . '</div>');
}
...
protected function _upload_file()
{
    ...
    if (isset($_FILES) && isset($_FILES['lnwphp-attach']) && !$_FILES['lnwphp-attach']['error'])
    {
...
protected function render_crop_window($filename, $field)
{
    ...
    $attr = array(
        ...
        'alt' => '');
    $out .= $this->single_tag('img', 'lnwphp-crop', $attr);
...
protected function _compare($val1, $operator, $val2)
{
    switch ($operator)
    {
        ...
        return ($val1 != $val2) ? true : false;
        case '^=':
        return (mb_strpos($val1, $val2, 0, lnwphp_config::$mbencoding) === 0) ? true : false;
        case '$=':
        return (mb_strpos($val1, $val2, 0, lnwphp_config::$mbencoding) == (mb_strlen($val1, lnwphp_config::$mbencoding) -
            mb_strlen($val2, lnwphp_config::$mbencoding))) ? true : false;
        case '~=':
        return (mb_strpos($val1, $val2, 0, lnwphp_config::$mbencoding) !== false) ? true : false;
        default:
        return false;
    }
}
protected function create_modal($field, $content, $image = false)
{
    ...
    $out .= $this->open_tag('a', 'lnwphp_modal', $attr);
    if (lnwphp_config::$images_in_grid && $image)
    {
        $out .= $content;
    }
    else
...
protected function _render_list_item($field, $value, $primary_val, $row)
{
    ...
    if (isset($this->field_type[$field]))
    {
        switch ($this->field_type[$field])
        {
            ...
            if ($value)
            {
                if (lnwphp_config::$images_in_grid)
                {
                    $settings = $this->upload_config[$field];
                    ...
                    $out .= $this->single_tag('img', '', array(
                        ...
                        'style' => 'max-height: ' . lnwphp_config::$images_in_grid_height . 'px;'));
                }
                else
                {
    ...
    if ($this->buttons)
    {
        foreach ($this->buttons as $button)
        {
            if ($this->is_button($button['name'], $row))
            {
                ...
                if (lnwphp_config::$button_labels)
                {
...
protected function file_link($field, $primary_val, $thumb = false, $crop = false, $filename = false)
{
    ...
    if ($thumb !== false)
    {
        $params['lnwphp']['thumb'] = $thumb;
    }
    if ($crop)
    {
        $params['lnwphp']['crop'] = $crop;
    }
    if (lnwphp_config::$dynamic_session)
    {
        $params['lnwphp']['sess_name'] = session_name();
    }
    return lnwphp_config::$scripts_url . '/' . lnwphp_config::$ajax_uri . '?' . http_build_query($params);
}
protected function html_safe($text)
{
    return htmlspecialchars((string )$text, ENT_QUOTES, lnwphp_config::$mbencoding);
}
...
protected function send_email($to, $subject = '(No subject)', $message = '', $cc = array(), $html = true)
{
    ...
    'From: ' . lnwphp_config::$email_from_name . ' <' . lnwphp_config::$email_from . ">\r\n";
    if ($cc)
...
protected function _cell_attrib($field, $value, $order, &$row, $is_sum = false, $row_color = false, $row_class = false)
{
    ...
    if ($field == $order && $this->is_sortable)
        $column_class[] = 'lnwphp-current';
    if ($is_sum)
        $column_class[] = 'lnwphp-sum';
    if ($row_color)
    {
...
{
    if (is_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/' . $this->language . '.ini'))
        $this->lang_arr = parse_ini_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/' . $this->language . '.ini');
    elseif (is_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/en.ini'))
        $this->lang_arr = parse_ini_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/en.ini');
    if ($this->set_lang)
    {
...
{
    if (is_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/' . lnwphp_config::$language . '.ini'))
        return parse_ini_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/' . lnwphp_config::$language . '.ini');
    elseif (is_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/en.ini'))
        $this->lang_arr = parse_ini_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/en.ini');
}
protected function _get_theme_config()
{
    if (is_file(lnwphp_PATH . '/' . lnwphp_config::$themes_path . '/' . $this->theme . '/lnwphp.ini'))
        $this->theme_config = parse_ini_file(lnwphp_PATH . '/' . lnwphp_config::$themes_path . '/' . $this->theme . '/lnwphp.ini');
    else
        self::error('lnwphp.ini does not exist in your theme folder');
}
protected function lang($text = '')
{
    $langtext = mb_convert_case($text, MB_CASE_LOWER, lnwphp_config::$mbencoding);
    return htmlspecialchars((isset($this->lang_arr[$langtext]) ? $this->lang_arr[$langtext] : $text), ENT_QUOTES,
        lnwphp_config::$mbencoding);
}
protected function theme_config($text = '')
{
    $text = mb_convert_case($text, MB_CASE_LOWER, lnwphp_config::$mbencoding);
    return htmlspecialchars((isset($this->theme_config[$text]) ? $this->theme_config[$text] : ''), ENT_QUOTES, lnwphp_config::
        $mbencoding);
}
...
public static function load_css()
{
    ...
    if (self::$css_loaded)
    {
        self::error('lnwphp\'s styles already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
    }

    self::$css_loaded = true;
    if (lnwphp_config::$load_bootstrap)
    {
        $out .= '<link href="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$plugins_uri .
        '/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />';
    }
    if (lnwphp_config::$load_jquery_ui)
        $out .= '<link href="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$plugins_uri .
    '/jquery-ui/jquery-ui.min.css" rel="stylesheet" type="text/css" />';
    if (lnwphp_config::$load_jcrop)
        $out .= '<link href="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$plugins_uri .
    '/jcrop/jquery.Jcrop.min.css" rel="stylesheet" type="text/css" />';
    $out .= '<link href="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$plugins_uri .
    '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' . lnwphp_config::$scripts_url .
    '/' . lnwphp_config::$themes_uri . '/' . lnwphp_config::$theme . '/lnwphp.css" rel="stylesheet" type="text/css" />';
    return $out;
}
public static function load_js()
{
    ...
    {
        $language = lnwphp_config::$language;
    ...
    if (self::$js_loaded)
    {
        self::error('lnwphp\'s scripts already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
    }
    self::$js_loaded = true;
    if (lnwphp_config::$load_jquery)
        $out .= '<script src="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$plugins_uri . '/jquery.min.js"></script>';
    if (lnwphp_config::$jquery_no_conflict)
    {
        $out .= '
...
if (lnwphp_config::$load_jquery_ui)
    $out .= '<script src="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$plugins_uri .
...
if (lnwphp_config::$editor_url)
    $out .= '<script src="' . lnwphp_config::$editor_url . '"></script>';
if (lnwphp_config::$load_googlemap)
    $out .= '<script src="//maps.google.com/maps/api/js?sensor=false&language=' . $language . '"></script>';
$out .= '<script src="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$plugins_uri . '/lnwphp.js"></script>';

$config = array(
    'url' => lnwphp_config::$scripts_url . '/' . lnwphp_config::$ajax_uri,
    'editor_url' => lnwphp_config::$editor_url,
    'editor_init_url' => lnwphp_config::$editor_init_url,
    'force_editor' => lnwphp_config::$force_editor,
    'date_first_day' => lnwphp_config::$date_first_day,
    'date_format' => lnwphp_config::$date_format,
    'time_format' => lnwphp_config::$time_format,
    'lang' => $lang_arr,
    'rtl' => lnwphp_config::$is_rtl ? 1 : 0);
$out .= '
<script type="text/javascript">
    <!--
    
    var lnwphp_config = ' . json_encode($config) . ';
    
-->
</script>';
if ($language != 'en')
{
    if (is_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/datepicker/jquery.ui.datepicker-' . $language . '.js'))
        $out .= '<script src="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$lang_uri .
    '/datepicker/jquery.ui.datepicker-' . $language . '.js"></script>';
    if (is_file(lnwphp_PATH . '/' . lnwphp_config::$lang_path . '/timepicker/jquery-ui-timepicker-' . $language . '.js'))
        $out .= '<script src="' . lnwphp_config::$scripts_url . '/' . lnwphp_config::$lang_uri .
...
protected function get_limit_list($limit = 20, $buttons = false)
{
    ...
    if ($buttons)
    {
        $out .= $this->open_tag(array(
            'tag' => 'div',
            'class' => 'btn-group lnwphp-limit-buttons',
        ...
        foreach ($this->limit_list as $limts)
        {
            ...
            $out .= $this->open_tag(array(
                ...
                'data-limit' => $limts), 'lnwphp-action') . $this->lang($limts) . $this->close_tag(array('tag' => 'button'));
        }
    ...
    else
    {
        $out .= $this->open_tag(array(
            ...
            'name' => 'limit'), 'lnwphp-actionlist lnwphp-data');
        foreach ($this->limit_list as $limts)
        {
...
{
    if (!lnwphp_config::$performance_mode)
    {
...
{
    if (!is_array($value) && !lnwphp_config::$performance_mode && $value)
    {
...
protected function send_http_request($url, $data, $method, $return_result = false)
{
    ...
    {
        case 'get':
        $ch = curl_init($path . ((mb_strpos($path, '?', 0, lnwphp_config::$mbencoding) === false) ? '?' : '&') . $data);
        break;
        case 'post':
        $ch = curl_init($path);
    ...
    if (lnwphp_config::$use_browser_info)
    {
    ...
    'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
{
    ...
    {
        $tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array('class' => 'lnwphp-tabs')) . $this->
        ...
        if ($this->default_tab !== false && $raw_out)
        {
            ...
            if (isset($this->field_tabs[$mode]) && $tabs_out)
            {
                ...
                if ($this->nested_rendered && lnwphp_config::$nested_in_tab)
                {
            ...
            protected function get_table_tooltip()
            {
                ...
                if ($this->table_tooltip)
                {
                    ...
                    $out .= $this->open_tag(array(
                        'tag' => 'a',
                        'href' => 'javascript:;',
                        'class' => 'lnwphp-tooltip lnwphp-button-link',
            ...
            protected function render_search()
            {
                ...
                if ($this->is_search)
                {
                    ...
                    $attr = array('class' => 'lnwphp-search-toggle', 'href' => 'javascript:;');
                    if ($this->search or lnwphp_config::$search_opened)
                    {
                        $attr['style'] = 'display:none;';
                    }
                    $out .= $this->open_tag('a', $this->theme_config('search_open'), $attr);
                    $out .= $this->lang('search') . $this->close_tag('a');
                    $attr = array('class' => 'lnwphp-search');
                    if (!$this->search && !lnwphp_config::$search_opened)
                    {
                        $attr['style'] = 'display:none;';
                    }
                    $out .= $this->open_tag('span', $this->theme_config('search_container'), $attr);
                    if (lnwphp_config::$search_all)
                    {
                    ...
                    $attr = array('class' => 'lnwphp-data', 'name' => 'column');
                    $out .= $this->open_tag('select', 'lnwphp-columns-select ' . $this->theme_config('search_fieldlist'), $attr);
                    $out .= implode('', $optlist);
            ...
            {
                $attr_preset = array('class' => 'lnwphp-searchdata', 'name' => 'phrase');
                if ($field == $this->column)
                {
                    $class = 'lnwphp-search-active';
                }
                else
                {
                    $class = '';
                ...
                switch ($attr['data-type'])
                {
                    ...
                    if (!isset($fieldlist['date']) or $field == $this->column)
                    {
                        ...
                        $attr_range = array(
                            'class' => 'lnwphp-daterange lnwphp-searchdata ' . $this->theme_config('search_range'),
                            'name' => 'range',
                            'data-fieldtype' => 'date');
                        if ($field != $this->column)
                        {
                        ...
                        close_tag('option');
                        if (lnwphp_config::$available_date_ranges)
                        {
                            foreach (lnwphp_config::$available_date_ranges as $range)
                            {
                        ...
                        $fieldlist['date'] .= $this->single_tag('input', 'lnwphp-datepicker-from ' . $class . ' ' . $this->theme_config('search_from'),
                            $attr);
                        $attr['name'] = 'phrase][to';
                        $attr['value'] = (isset($phrase['to']) && $field == $this->column) ? $phrase['to'] : '';
                        $fieldlist['date'] .= $this->single_tag('input', 'lnwphp-datepicker-to ' . $class . ' ' . $this->theme_config('search_to'),
                            $attr);
                    }
                    break;
                    case 'select':
            ...
            {
                $out = '';
                $out .= $this->open_tag($row, 'lnwphp-th');
                if ($this->is_numbers)
                {
                    $out .= $this->open_tag($item, 'lnwphp-num') . '&#35;' . $this->close_tag($item);
                }
                ...
                {
                    $out .= $this->open_tag($item, 'lnwphp-actions') . '&nbsp;' . $this->close_tag($item);
                }
                foreach ($this->columns as $field => $fitem)
                {
                    if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' or $this->field_type[$field] ==
                    ...
                    if ($this->is_sortable)
                    {
                        ...
                        {
                            $class .= ' lnwphp-primary';
                        }
                        if ($this->order_column == $field)
                        {
                            $class .= ' lnwphp-current lnwphp-' . $this->order_direct;
            ...
            protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
            {
                ...
                if ($this->result_list)
                {
                    foreach ($this->result_list as $key => $row)
                    {
                        ...
                        $out .= $this->open_tag($row_tag, 'lnwphp-row lnwphp-row-' . $i);
                        if ($this->is_numbers)
                        {
                            $out .= $this->open_tag($item, 'lnwphp-num', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class)) . ($key +
                                $this->start + 1) . $this->close_tag($item);
                        }
                        if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                            grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'left')
                        {
                            $out .= $this->open_tag($item, 'lnwphp-actions', $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
                        ...
                        if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                            grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right')
                        {
                            $out .= $this->open_tag($item, 'lnwphp-actions' . (lnwphp_config::$fixed_action_buttons ? ' lnwphp-fix' : ''), $this->
                ...
                else
                {
                    ...
                    $out .= $this->open_tag($row_tag, 'lnwphp-row') . $this->open_tag($item, '', array('colspan' => $j)) . $this->lang('table_empty') .
            ...
            protected function render_grid_footer($row = array('tag' => 'tr'), $item = array('tag' => 'td'))
            {
                ...
                {
                    $out .= $this->open_tag($row, 'lnwphp-tf');
                    if ($this->is_numbers)
                    {
                        $out .= $this->open_tag($item, 'lnwphp-num') . '&Sigma;' . $this->close_tag($item);
                    }
            ...
            protected function render_benchmark($tag = array('tag' => 'span'))
            {
                if ($this->benchmark)
                {
                    return $this->open_tag($tag, 'lnwphp-benchmark') . $this->benchmark_end() . $this->close_tag($tag);
                }
                return '';
            }
            protected function render_control_fields()
            {
                ...
                $out .= $this->single_tag($tag, '', array('name' => 'limit', 'value' => ($this->limit ? $this->limit : lnwphp_config::$limit)));
            ...
            protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
            {
                ...
                if (!isset($this->hide_button[$name]))
                {
                    ...
                    $out .= $this->open_tag($tag, 'lnwphp-action');
                    if ($icon && !$this->is_rtl)
                    {
            ...
            protected function csv_button($class = '', $icon = '')
            {
                ...
                {
                    return $this->render_button('export_csv', 'csv', '', $class . ' lnwphp-in-new-window', $icon);
                }
            }
            protected function print_button($class = '', $icon = '')
            {
                ...
                {
                    return $this->render_button('print', 'print', '', $class . ' lnwphp-in-new-window', $icon);
                }
            }
            ...
            protected function get_image_folder($field)
            {
                ...
                {
                    $path = $this->check_folder(lnwphp_config::$upload_folder_def, 'get_image_folder');
                }
            ...
            protected function is_resize($field)
            {
                if (isset($this->upload_config[$field]['width']) or isset($this->upload_config[$field]['height'])
                 or (isset($this->upload_config[$field]['manual_crop']) && $this->
                    upload_config[$field]['manual_crop'] == true) )
                    return true;
                else
                    return false;
            }
            ...
            protected function get_range($name)
            {
                ...
                switch ($name)
                {
                    ...
                    if ($week_day >= lnwphp_config::$date_first_day)
                    {
                        $offset1 = $week_day - lnwphp_config::$date_first_day;
                    }
                    else
                    {
                        $offset1 = 7 - (lnwphp_config::$date_first_day - $week_day);
                    }
                    ...
                    $offset2 = 6 - $week_day + lnwphp_config::$date_first_day;
                    $range['from'] = mktime(0, 0, 0, date('n', $time), date('j', $time) - $offset1, date('Y', $time));
            ...
            {
                if ($time)
                    return $utc ? gmdate(lnwphp_config::$php_date_format, $time) : date(lnwphp_config::$php_date_format, $time);
                else
                    return '';
            }
            ...
            {
                if ($time)
                    return $utc ? gmdate(lnwphp_config::$php_date_format . ' ' . lnwphp_config::$php_time_format, $time) : date(lnwphp_config::
                        $php_date_format . ' ' . lnwphp_config::$php_time_format, $time);
                else
                    return '';
            }
            ...
            {
                if ($time)
                    return $utc ? gmdate(lnwphp_config::$php_time_format, $time) : date(lnwphp_config::$php_time_format, $time);
                else
                    return '';
            }
            ...
            protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
            {
                ...
                if ($this->is_title)
                {
                    ...
                    if ($to_show)
                        $out .= '<span class="lnwphp-toggle-show lnwphp-toggle-down"><i class="' . $this->theme_config('slide_down_icon') .
                    '"></i></span>';
                    else
                        $out .= '<span class="lnwphp-toggle-show lnwphp-toggle-up"><i class="' . $this->theme_config('slide_up_icon') .
                    '"></i></span>';
                    $out .= $this->close_tag($tag);
                }
                return $out;
            ...
            {
                if (!lnwphp_config::$alt_encription_key)
                {
                ...
                $key = substr(lnwphp_config::$alt_encription_key, 0, $ks);
            ...
            {
                $_SESSION['lnwphp_session'] = $data;
            }
            public static function export_session()
            {
                return $_SESSION['lnwphp_session'];
            }
            ...
            {
                $path = lnwphp_PATH . '/core/' . $name . '.php';
        ...
        class lnwphp_postdata
        {
            private $lnwphp = null;
            private $postdata = array();
            public function __construct($postdata, $lnwphp)
            {
                $this->lnwphp = $lnwphp;
            ...
            {
                $fdata = $this->lnwphp->_parse_field_names($name, 'lnwphp_postdata');
                foreach ($fdata as $key => $fitem)
                {
                    $this->postdata[$key] = $value;
                }
                $this->lnwphp->unlock_field($name);
                return $this;
            }
            public function get($name)
            {
