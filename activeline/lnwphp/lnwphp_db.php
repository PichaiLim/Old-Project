...
class lnwphp_db
{
    ...
    public static function get_instance($params = false)
    {
        ...
        if (!isset(self::$_instance[$instance_name]) or null === self::$_instance[$instance_name])
        {
            if (!is_array($params))
            {
                $dbuser = lnwphp_config::$dbuser;
                $dbpass = lnwphp_config::$dbpass;
                $dbname = lnwphp_config::$dbname;
                $dbhost = lnwphp_config::$dbhost;
                $dbencoding = lnwphp_config::$dbencoding;
            }
    ...
    private function __construct($dbuser, $dbpass, $dbname, $dbhost, $dbencoding)
    {
        ...
        if (!$this->connect)
            $this->error('ไม่สามารถเชื่อมต่อ ฐานข้อมูลได้');
        $this->connect->set_charset($dbencoding);
        if ($this->connect->error)
            $this->error($this->connect->error);
        if (lnwphp_config::$db_time_zone)
            $this->connect->query('SET time_zone = \'' . lnwphp_config::$db_time_zone . '\'');
    }
