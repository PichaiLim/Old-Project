    ...
    public function accessRules()
    {
        return array(
            array('allow',  // allow all users to perform 'index' and 'view' actions
                'actions'=>array('index','view', 'ViewLastInventoryPushRecieptNo'),
//				'users'=>array('*'),
...
//				'users'=>array('admin'),
    ...
    public function actionViewLastInventoryPushRecieptNo(){
    ...
    public function actionSetCreate() {
        ...
        if($model->save() == true){
            ...
            for($i=0; $i < count($data['detail']); $i++){
                $modelDetail = new InventoryPushDetail;
                $modelDetail->inventory_push_id = $model->reciept_no;
        ...
        for($a=0; $a < count($data['detail']); $a++){
            ...
            if($InventoryFindPK != null){
                ...
                $modelInventory->pushed = new CDbExpression('NOW()');
                ...
                $newInventory->product_id = $data['detail'][$a]['product_id'];
                $newInventory->pushed = new CDbExpression('NOW()');
                $newInventory->pushed_by = $data['created_by'];
                $newInventory->quantity = $data['detail'][$a]['quantity'];

                if($newInventory->save() == true){
                    $response['success-inventory-update'] = true;
                    $response['errors-inventory-update'] = $newInventory->errors;
                    $response['model-inventory-update']['inventory'] = $newInventory;
                }else{
                    $response['success-inventory-update'] = false;
                    $response['errors-inventory-update'] = $newInventory->errors;
                    $response['model-inventory-update'] = $newInventory;
                }
            }
        }
    ...
    private function getLastRecieptNo(){
        ...
        $sql    .= "FROM inventory_push AS i ";
        $sql    .= " WHERE i.reciept_no LIKE '%INV{$yearNow2DigiTh}%' ";
        ...
        if($query['reciept_no'] != ""){
            ...
            $newNo = "INV".$yearNow2DigiTh.'-000001';
        }
