	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view', 'list', 'BranchListAll'),
//				'users'=>array('?'),
                'expression'=>"Yii::app()->user->name !== 'Guest' ",
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','SetCreate', 'SetUpdate'),
//				'users'=>array('@'),
                'expression'=>"Yii::app()->user->name !== 'Guest' ",
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
...
//				'users'=>array('@'),
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete', 'DeleteBranch'),
				'expression'=>"Yii::app()->user->name !== 'Guest' ",
			),
	...
	public function actionBranchListAll(){
        $get = file_get_contents('php://input');
        $data = CJSON::decode($get);

        $sql = "SELECT b.id,b.name,(case when b.active = '1' then 'เปิดการใช้งาน' else 'ปิดการใช้งาน' end) as active ,b.created,(select e.username from employee e where e.id= b.created_by) as created_by,b.updated,(select e.username from employee e where e.id=b.updated_by) as updated_by FROM branch b";
	...
	public function actionDeleteBranch(){
        ...
        $data = CJSON::decode($post,true);
        $branch=Branch::model()->findByPk($data['id']); // assuming there is a post whose ID is 10
		$branch->delete();
	}
