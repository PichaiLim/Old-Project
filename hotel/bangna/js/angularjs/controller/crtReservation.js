...
app.controller('ReservationController', function($scope, $location, $http, $routeParams, ReservationService,
    $uibModal, DateUtilService, $filter, URLREPORTDAILY, URLREPORTSUMMARYDAILY) {
    var $scope = this;
    ...
    $scope.init = function(pageId, branchID) {
        ...
        if (pageId == 2) {
            ...
            $scope.urlReportDaily = URLREPORTDAILY;
            $scope.getReportDailyAllList('','');
        }else if(pageId == 4){
            $scope.urlReportSummaryDaily = URLREPORTSUMMARYDAILY;
            $scope.getReportSummaryDailyAllList('','');
        } else {
            $scope.building();
        }
    };
    ...
    $scope.openPrintModal = function(start, end, url) {
        $uibModal.open({
            animation: true,
            templateUrl: 'print_modal_content.html',
    ...
    function setTemplateMapping(type, reservationId) {
        var templateMapping = {
            ...
            "7": "manageRoomReadModalContent.html"
    ...
    function updateDeposit(dataRoom, type, reservationId, page) {
        return function(data) {
            if (data.give_paid_status === 'no' && (type === 3  || type === 5)) {
    ...
    function saveReservedOrHistory(data, dataRoom, type, reservationId, page) {
        if (reservationId) {
            ...
            ReservationService.saveReservation(getDataReservationForSave(data, dataRoom, branch_id))
                .then(updateRoomStatus(dataRoom.floor_id, page, data));
        }
    }
    ...
    function updateReservation(data, reservationId, floor_id, type, page) {
        return function() {
            ReservationService.updateReservation(getDataReservationForUpdate(data, reservationId, type))
                .then(updateRoomStatus(floor_id, page, data));
        };
    }

    function updateRoomStatus(floor_id, page, data) {
        return function(response) {
            if (data.paid_status == 'yes' && (data.status == 'reserved' || data.status == 'checkin')) {
                $scope.openRecieptPrint(response.data.models.id);
            }
            if (page != 1) {
                $scope.getReservationDataAllList();
            } else {
                if ((data.paid_status == 'no' && data.status == 'checkin') || data.status == 'cancelled') {
                    window.location.reload(true);
                }
            }
        };
    ...
    $scope.getReservationDataAllList = function() {
        ReservationService.getReservationDataAll({
                branch_id: branch_id
            })
            .then(function(data) {
     ...
     $scope.getReportDailyAllList = function(start,end) {
        var startDate = start || "";
        var endDate = end || "";
        var data = {
            branch_id: branch_id,
        ...
        ReservationService.getReportDaily(data)
            .then(function(data) {
                $scope.setReportDailyAllList = data;
            });
    };

    $scope.getReportSummaryDailyAllList = function(start,end) {
        var startDate = start || "";
        var endDate = end || "";
        var data = {
            branch_id: branch_id,
        ...
        ReservationService.getReportSummaryDaily(data)
            .then(function(data) {
                $scope.setReportSummaryDailyAllList = data;
            });
    };

    $scope.calStartDateAndEndDate = function(start, end){
        return DateUtilService.dateDiff(start, end);
    };
    ...
    function getDataReservationHistoryForSave(data) {
        return {
            ...
            amount: data.amount || "",
            paid_deposit: data.paid_deposit || "",
        ...
        return function(roomList) {
            ...
            for (var i = 0; i < reservationList.length; i++) {
                if (roomList.id === reservationList[i].room_id) {
                    if (!$scope.validateEnddateToEqualSelectDate(reservationList[i].end)) {
    ...
    $scope.validateEnddateToEqualSelectDate = function(end) {
        return end === $scope.datePicker;
...
app.controller('PrintModalController', function($scope, $uibModalInstance, modalData) {
    $scope.url = modalData.url + "?branch=" + modalData.branchId + "&start=" + modalData.startDate + "&endDate=" + modalData.endDate;
...
app.controller('ManageRoomModalController', function($scope, $uibModalInstance, modalData, ReservationService, $uibModal, DateUtilService, CustomerService) {
    ...
    $scope.searchCustomerModal = function() {
        $uibModal.open({
            ...
            if (parseFloat($scope.depositOld) >= parseFloat(modalData.dataRoom.roomTypeDeposit)) {
                $scope.amount = parseFloat(modalData.dataRoom.roomTypePrice);
            } else {
                var calDeposit = parseFloat(modalData.dataRoom.roomTypeDeposit) - $scope.depositOld;
                $scope.amount = (parseFloat(modalData.dataRoom.roomTypePrice) + parseFloat(calDeposit));
            }
    ...
    $scope.init = function() {
        ...
        $scope.depositBackup = parseFloat(modalData.dataRoom.roomTypeDeposit);
        ...
        if (modalData.reservationId) {
            ReservationService.getReservationByPk(modalData.reservationId).then(function(response) {
                ...
                $scope.depositBackup = dataReservation.deposit;
    ...
    $scope.ok = function(userId) {
        ...
        if ($scope.data.type == roomMoveType) {
            ...
            if (parseFloat($scope.depositOld) >= parseFloat($scope.deposit)) {
                depositUpdate = parseFloat($scope.depositOld) - parseFloat($scope.deposit);
                used_deposit_old = parseFloat($scope.deposit);
            } else {
                depositUpdate = 0;
                used_deposit_old = parseFloat($scope.depositOld);
            }
        }else if($scope.data.type == roomCancelledType || $scope.data.type == roomCancelledReservationype){
            depositUpdate = parseFloat($scope.depositOld);
        }
    ...
    $scope.isPaidDeposit = function() {
        if ($scope.paid_deposit === 'yes') {
            ...
            $scope.deposit = parseFloat($scope.depositBackup);
        }
        calulateaAmount();
    };
    ...
    $scope.calulateDate = function() {
        if ($scope.start && $scope.end) {
            ReservationService.validateDateRoomReservedOrCheckin({
                roomId: modalData.dataRoom.room_id,
                startDate: $scope.start,
                endDate: $scope.end
            }).then(function(data) {
                $scope.dataRoomCheckinOrResered = data;
            });
            calulateaAmount();
        }
    };

    $scope.calulateDiscount = function() {
        ...
        if ($scope.discount) {
            $scope.amount = priceAllDay - parseFloat($scope.discount);
        } else {
            calulateaAmount();
        }
    }
    ...
    function calulateaAmount() {
        ...
        if (parseFloat($scope.depositOld) >= parseFloat($scope.deposit)) {
            $scope.amount = (parseFloat($scope.num_days) * parseFloat($scope.price));
        } else {
            var calDepositA = parseFloat($scope.deposit) - parseFloat($scope.depositOld);
            $scope.amount = (parseFloat($scope.num_days) * parseFloat($scope.price)) + parseFloat(calDepositA);
        }
