...
app.controller('RoomController', function($scope, RoomService, $routeParams, $http){
    ...
    */
    ...
    $scope.SaveUpdate = function(){
        ...
        // Send update data
        RoomService.setRoomUpdate(data).then(function(req){
            if(req.status === 200){
    ...
    $scope.confirmDelete = function(id){
        RoomService.getRoomDelete(id).then(function(data){
            if(data.status === 200){
                window.location.reload(false);
            }
            return false;
